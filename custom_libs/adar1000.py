...
class adar1000_array(context_manager):
    ...
    def _get_element_number(self, element) -> int:
        """
        Extract the element number so we can look it up in the calibration dict.
        Prefer a real attribute if your Element class has one; otherwise fall back.
        """
        for attr in ("number", "id", "element_no", "elem_id"):
            if hasattr(element, attr):
                return int(getattr(element, attr))
        # Fallback to your previous "last two digits in str(element)" behavior
        s = str(element)
        digits = ''.join(ch for ch in s if ch.isdigit())
        if digits:
            return int(digits[-2:])
        raise ValueError(f"Cannot derive element number from: {s}")
    def __repr__(self):
        """ Representation of the ADAR1000 array class """
        return f"ADAR1000 array containing {len(self.devices)} ADAR1000 instances:\n\t{', '.join(self._chip_ids)}"
    def _wrap_0_360(self, phi_deg: float) -> float:
    ...
    def _steer(self, rx_or_tx, azimuth, elevation, cal_dict=None, cal_is_measured_error=False):
        """
        Steer the array. Optionally include per-element calibration from a dict.
        ...
        cal_dict : dict | None
            Optional dict {element_number: phase_value_in_degrees}.
            If provided, each element's phase command will be:
                phase = row_phase + column_phase + cal_term      (all in degrees)
            where 'cal_term' comes from this dict (after optional sign handling).
        cal_is_measured_error : bool
            If True, the values in cal_dict are measured errors (target - ref),
            so the correction to apply is the *negative* of that (mod 360).
            If False, values are already corrections to add as-is.
        """
        rx_or_tx = rx_or_tx.strip().lower()
        if rx_or_tx not in ("rx", "tx"):
            raise ValueError(f"rx_or_tx must be 'rx' or 'tx', got '{rx_or_tx}'")

        # Compute per-axis phase increments (degrees per index)
        ...
        rows = np.array([element.row for element in elements])
        columns = np.array([element.column for element in elements])

        # Load per-element calibration if provided
        if cal_dict is not None:
            cal_terms = np.array([float(cal_dict.get(self._get_element_number(element), 0.0)) for element in elements])
        else:
            cal_terms = np.zeros(len(elements))

        # Compute phases
        row_phases = rows * elevation_phi
        column_phases = columns * azimuth_phi
        phase_cmds = self._wrap_0_360(row_phases + column_phases + cal_terms)

        # Clamp near-zero phases
        adjusted_cmds = [0.0 if p < 0.01 else float(p) for p in phase_cmds]

        # Group writes by parent device.  Elements on different ADAR1000 chips
        # map to independent IIO device nodes so concurrent writes are safe.
        _dev_items = defaultdict(list)
        for element, phase_cmd in zip(elements, adjusted_cmds):
            _dev_items[element.adar1000_parent].append((element, phase_cmd))

        def _write_device(items):
            for elem, cmd in items:
                if rx_or_tx == "rx":
                    elem.rx_phase = cmd
                else:
                    elem.tx_phase = cmd

        def _latch_device(device):
            if rx_or_tx == "rx":
                device.latch_rx_settings()
            else:
                device.latch_tx_settings()

        # Write all devices in parallel, then latch all in parallel.
        with ThreadPoolExecutor(max_workers=len(_dev_items)) as pool:
            list(pool.map(_write_device, _dev_items.values()))
            # All element writes are guaranteed complete before latching.
            list(pool.map(_latch_device, _dev_items.keys()))
    ...
    def steer_rx(self, azimuth, elevation, cal_dict=None, cal_is_measured_error=False):
        ...
        self._steer("rx", azimuth, elevation, cal_dict=cal_dict, cal_is_measured_error=cal_is_measured_error)

    def steer_tx(self, azimuth, elevation, cal_dict=None, cal_is_measured_error=False):
        ...
        self._steer("tx", azimuth, elevation, cal_dict=cal_dict, cal_is_measured_error=cal_is_measured_error)

    # ── Subarray-to-element mapping (matches hardware_setup.py) ──────
    SUBARRAY_ELEMENTS = {
        1: [1, 2, 5, 6, 9, 10, 13, 14],
        2: [3, 4, 7, 8, 11, 12, 15, 16],
        3: [19, 20, 23, 24, 27, 28, 31, 32],
        4: [17, 18, 21, 22, 25, 26, 29, 30],
    }

    SUPPORTED_TAPERS = ["uniform", "hamming", "hanning", "blackman", "taylor", "chebyshev"]

    def apply_taper(self, taper_type="uniform", scale_existing=False, subarray_modes=None):
        """Apply an amplitude taper (window) across the array elements.

        Parameters
        ----------
        taper_type : str
            One of SUPPORTED_TAPERS.
        scale_existing : bool
            If True, multiply taper weights onto the current gain values
            (preserving calibration).  If False, set gains directly from
            the taper (0-127 range).
        subarray_modes : dict | None
            {subarray_number: "rx"/"tx"/"off"}.  When provided the taper
            is computed over the active RX aperture only, so the window
            shape matches the physical geometry.
        """
        taper_type = taper_type.strip().lower()
        if taper_type not in self.SUPPORTED_TAPERS:
            raise ValueError(f"Unsupported taper '{taper_type}'. Choose from {self.SUPPORTED_TAPERS}")

        # Determine which elements are active RX
        if subarray_modes is not None:
            active_elements = []
            for sa, mode in subarray_modes.items():
                if mode.lower() == "rx":
                    active_elements.extend(self.SUBARRAY_ELEMENTS.get(sa, []))
            active_elements.sort()
        else:
            active_elements = sorted(self.elements.keys())
        ...
        if taper_type == "uniform":
            ...
            weights = taylor(n, nbar=4, sll=30, norm=True)
        elif taper_type == "chebyshev":
            from scipy.signal.windows import chebwin
            weights = chebwin(n, at=60)
        else:
            weights = np.ones(n)
        ...
        # Apply to elements
        for idx, elem_num in enumerate(active_elements):
            if elem_num not in self.elements:
                continue
            element = self.elements[elem_num]
            if scale_existing:
                current_gain = element.rx_gain
                element.rx_gain = int(np.clip(round(current_gain * weights[idx]), 0, 127))
            else:
                element.rx_gain = int(np.clip(round(127 * weights[idx]), 0, 127))
